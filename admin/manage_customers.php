 ...
 *
 * Admin - Manage Customers (End-Users)
 */
...
if (isset($_GET['ajax_get_prices'])) {
    ...
    // KEY CHANGE: Select 'price_user' as the base price for customers.
    $sql = "
        SELECT p.id, p.name, p.price_user, upp.price as custom_price
        FROM products p
            ...
            <tbody>
                <?php while ($product = $products->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td class="text-center"><strong>₹<?php echo number_format($product['price_user'], 2); ?></strong></td>
                        ...
                        <td>
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" class="form-control" 
                                       ...
                                       placeholder="<?php echo number_format($product['price_user'], 2); ?>"
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'set_prices') {
        ...
        $conn->begin_transaction();
        try {
            // KEY CHANGE: Validate against 'price_user'
            $cost_stmt = $conn->prepare("SELECT price_user FROM products WHERE id = ?");
            ...
            foreach ($prices as $product_id => $price_str) {
                ...
                if ($price <= 0) {
                    ...
                    if ($product && $price < $product['price_user']) {
                        throw new Exception("Custom price cannot be lower than the base price of ₹" . number_format($product['price_user'], 2));
                    }
            ...
            $conn->commit();
            $_SESSION['success_message'] = 'Custom prices for customer updated successfully!';
        } catch (Exception $e) {
            $conn->rollback();
...
// --- DATA FETCHING for the main list (Fetches 'user' role) ---
$users_result = $conn->query("SELECT u.id, u.username, u.email, u.status, w.balance FROM users u LEFT JOIN wallets w ON u.id = w.user_id WHERE u.role = 'user' ORDER BY u.username ASC");
...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid"><h1 class="m-0">Manage Customers</h1></div>
    </section>
        ...
        <div class="container-fluid">
            <div class="card card-outline card-primary shadow-sm">
                <div class="card-header card-header-action">
                    <h3 class="card-title"><i class="fas fa-user me-2"></i>All Customer Accounts</h3>
                    <a href="add_user.php?role=user" class="btn btn-primary"><i class="fas fa-plus me-1"></i> Create New Customer</a>
                </div>
                <div class="card-body">
...
$(function () {
    ...
    $('#usersTable tbody').on('click', '.set-pricing-btn', function(e) {
        ...
        $.get('manage_customers.php', { ajax_get_prices: 1, user_id: userId })
