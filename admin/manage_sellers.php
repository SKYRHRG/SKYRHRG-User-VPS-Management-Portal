 ...
 *
 * Admin - Manage Sellers (Fully Integrated Modal UI)
 */
...
if (isset($_GET['ajax_get_prices'])) {
    ...
    // KEY CHANGE: Select 'price_seller' as the base price for validation.
    $sql = "
        SELECT p.id, p.name, p.price_seller, upp.price as custom_price
        FROM products p
            ...
            <tbody>
                <?php while ($product = $products->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td class="text-center"><strong>₹<?php echo number_format($product['price_seller'], 2); ?></strong></td>
                        ...
                        <td>
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" class="form-control" 
                                       ...
                                       placeholder="<?php echo number_format($product['price_seller'], 2); ?>"
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'adjust_balance') {
        ...
        if ($user_id > 0 && $amount > 0 && !empty($description) && in_array($adjustment_type, ['credit', 'debit'])) {
            ...
            $_SESSION['error_message'] = 'Invalid data for balance adjustment.';
        }
        redirect('manage_sellers.php');
    }
    ...
    if ($action === 'set_prices') {
        ...
        $conn->begin_transaction();
        try {
            // KEY CHANGE: Validate against 'price_seller'
            $cost_stmt = $conn->prepare("SELECT price_seller FROM products WHERE id = ?");
            ...
            foreach ($prices as $product_id => $price_str) {
                ...
                if ($price <= 0) {
                    ...
                    if ($product && $price < $product['price_seller']) {
                        throw new Exception("Custom price cannot be lower than the base price of ₹" . number_format($product['price_seller'], 2));
                    }
            ...
            $conn->commit();
            $_SESSION['success_message'] = 'Custom prices for seller updated successfully!';
        } catch (Exception $e) {
            $conn->rollback();
...
// --- DATA FETCHING for the main list (Fetches 'seller' role) ---
$users_result = $conn->query("SELECT u.id, u.username, u.email, u.status, w.balance FROM users u LEFT JOIN wallets w ON u.id = w.user_id WHERE u.role = 'seller' ORDER BY u.username ASC");
...
<style>
    ...
    .user-info .user-name { font-weight: 600; color: #333; display: block; }
    .user-info .user-email { font-size: 0.85rem; color: #6c757d; }
    .actions-dropdown .dropdown-toggle::after { display: none; }
    .actions-dropdown .btn { border-radius: 50px; }
    .card-header-action { display: flex; justify-content: space-between; align-items: center; }
    .modal-header { background-color: #f8f9fa; border-bottom: 1px solid #dee2e6; }
    .form-control-icon { position: relative; }
    .form-control-icon .form-icon { position: absolute; top: 50%; left: 1rem; transform: translateY(-50%); color: #6c757d; }
    .form-control-icon .form-control { padding-left: 2.8rem; }
</style>
...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid"><h1 class="m-0">Manage Sellers</h1></div>
    </section>
        ...
        <div class="container-fluid">
            <div class="card card-outline card-primary shadow-sm">
                <div class="card-header card-header-action">
                    <h3 class="card-title"><i class="fas fa-user-tag me-2"></i>All Seller Accounts</h3>
                    <a href="add_user.php?role=seller" class="btn btn-primary"><i class="fas fa-plus me-1"></i> Create New Seller</a>
                </div>
                <div class="card-body">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="dropdown actions-dropdown">
                                            <button class="btn btn-sm btn-light dropdown-toggle" type="button" data-bs-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button>
...
<div class="modal fade" id="adjustBalanceModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content">
    ...
    <form action="manage_sellers.php" method="post">
        <div class="modal-body">
            <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>"><input type="hidden" name="action" value="adjust_balance"><input type="hidden" name="user_id" id="modal_user_id_balance">
...
<div class="modal fade" id="pricingModal" tabindex="-1"><div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content">
    ...
    <form action="manage_sellers.php" method="post">
        <div class="modal-body">
            <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>"><input type="hidden" name="action" value="set_prices"><input type="hidden" name="user_id" id="modal_user_id_pricing">
...
$(function () {
    const table = $('#usersTable').DataTable({
        ...
        "language": { "emptyTable": "No sellers found." },
        "columnDefs": [
            { "orderable": false, "targets": 3 }, { "className": "dt-center", "targets": [2, 3] }, { "className": "dt-right", "targets": 1 }
        ]
    });
    ...
    $('#usersTable tbody').on('click', '.set-pricing-btn', function(e) {
        ...
        $.get('manage_sellers.php', { ajax_get_prices: 1, user_id: userId })
