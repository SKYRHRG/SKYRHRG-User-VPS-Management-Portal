...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($recipient_id > 0 && $amount > 0) {
        ...
        try {
            // 1. Check sender's balance (with locking for concurrency)
            ...
            if ($sender_balance < $amount) {
                throw new Exception("You do not have sufficient balance for this transfer.");
            }

            // 2. Verify recipient is a seller created by this super seller
            $verify_stmt = $conn->prepare("SELECT id FROM users WHERE id = ? AND role = 'seller' AND created_by = ?");
            $verify_stmt->bind_param("ii", $recipient_id, $current_user_id);
            $verify_stmt->execute();
            if ($verify_stmt->get_result()->num_rows === 0) {
                throw new Exception("Invalid recipient. You can only transfer to sellers you have created.");
            }
            $verify_stmt->close();
            ...
            // 4. Log transactions for both users
            $desc_sender = "Transferred to Seller ID $recipient_id. " . $description;
            $desc_recipient = "Received from Super Seller ID $current_user_id. " . $description;
...
// --- DATA FETCHING ---
// Fetch sellers created by this super seller
$sellers_stmt = $conn->prepare("SELECT id, username FROM users WHERE role = 'seller' AND created_by = ?");
$sellers_stmt->bind_param("i", $current_user_id);
            ...
            <div class="row">
                <div class="col-lg-8 mx-auto">
                    <div class="card card-primary">
                        <div class="card-header"><h3 class="card-title">Transfer Funds to a Seller</h3></div>
                        <form action="transfer_balance.php" method="post">
                            <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>">
                            <div class="card-body">
                                <div class="form-group">
                                    <label>Recipient Seller</label>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="description" placeholder="e.g., Monthly commission">
                                </div>
                            </div>
